...
function Modal({ open = false, onClose, channel, saveChannel }) {
  ...
  useEffect(() => {
    ...
    if (channel) {
      //recieve calibration reults via socket
      socket.on(
        SOCKET_EVENTS.CALIBRATION_COMPLETE,
        handleReceiveCalibrationData
      );
    }else{
        socket.off(SOCKET_EVENTS.CALIBRATION_COMPLETE);
    }

    return () => socket.off(SOCKET_EVENTS.CALIBRATION_COMPLETE);
  }, [channel]);
  ...
  function handleReceiveCalibrationData({data}) {
    console.log("Calibration result:");
    console.log(data)
    const temp = { ...formikRef.current.values };
    temp.Sensor.Sensitivity = data.scale_factor;
    saveChannel(channel.Channel.Port, temp);
    formikRef.current.setFieldValue("Sensor.Sensitivity", data.scale_factor);
          ...
          <Formik {...formik} innerRef={formikRef}>
            {(props) => (
              <>
                <div>
                  ...
                  >
                    <Grid item>
                      ...
                      <Select
                        error={
                          props?.errors?.Coupling && props.touched?.Coupling
                        }
                    ...
                    <Grid item>
                      ...
                      <Select
                        error={
                          props?.errors?.InputRange && props.touched?.InputRange
                        }
                ...
                <h2 style={{ marginTop: "2rem" }}>Калибровка канала</h2>
                <div className="calibration_form">
                  <div>
                    <InputLabel>Эталонная амплитуда</InputLabel>
                    <TextField
                      value={props.values.calibration.standard_amplitude}
                      placeholder="10"
